...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("Connect again %d", leader1)
	...
	// be elected.
	DPrintf("Disconnect %d and %d", leader2, (leader2+1)%servers)
	cfg.disconnect(leader2)
	cfg.disconnect((leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	DPrintf("Connect again %d", (leader2+1)%servers)
...
// 	DPrintf("Disconnect %d server now", (leader+1)%servers)
...
// 	DPrintf("Connect %d server again", (leader+1)%servers)
...
// 	// all together now
// 	cfg.connect(leader2)
// 	DPrintf("Connect again %d ", leader2)
...
func TestBackup2B(t *testing.T) {
	...
	DPrintf("Disconnect nodes: %d %d %d", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	DPrintf("Disconnect nodes: %d %d", (leader1+0)%servers, (leader1+1)%servers)

	// allow other partition to recover
	cfg.connect((leader1 + 2) % servers)
	cfg.connect((leader1 + 3) % servers)
	cfg.connect((leader1 + 4) % servers)
	DPrintf("Connect again nodes: %d %d %d", (leader1+2)%servers, (leader1+3)%servers, (leader1+4)%servers)
	...
	cfg.connect(other)
	DPrintf("Connect again nodes: %d %d %d", (leader1+0)%servers, (leader1+1)%servers, other)
